...
%     dopplerIdx = rd_peak_list(2, :);
    ...
    if cfgOut.applyVmaxExtend 
        ...
        deltaPhi = 2 * pi * (dopplerInd_unwrap - ChirpNum / 2 - 1) / (numTx * ChirpNum); % 多普勒相位修正 tarNum * TXNUM 
        ...
        delta_phi_noapply = reshape(2 * pi * (dopplerIdx - ChirpNum / 2 - 1) / (numTx * ChirpNum),[],1);
...
%         [max_val, max_idx] = max(max(abs(sig_bin_row1_fft_cut), [], 2), [], 3);
        ...
        sig_bin_org = doaInput; % TARNUM * RXNUM * TXNUM
        deltaPhi = 2 * pi * (dopplerIdx - ChirpNum / 2 - 1) / (numTx * ChirpNum); % 多普勒相位修正 tarNum * 1
        deltaPhi = deltaPhi.';
